...
public class GraphMaker {
     ...
     * @param b number of vertices to add
     */
    public static float growWeightedTree(AbstractGraph G, Vertex v, Random r, int b, float maxW) {
        Vertex prev = v;
        float weigth = 0.0f;
        for (int i = 0; i < b; i++) {
            Vertex br = G.addVertex(v.getLabel() + i);
            float we = randomWeight(r, maxW, 0.1f);
            weigth += we;
            prev.addEdge(br, "E" + prev.getLabel() + br.getLabel(), Graph.WHITE, we);
            if ((i % 2) == 0 || r.nextBoolean())
                prev = br;
        }
        return weigth;
     ...
     * @param r  random number generator
     * @param vn number of additional vertices (at least 2)
     * @param en approximate number of additional edges (at least vn+1)
     */
    public static void makeNonTree(Graph G, Vertex v, Random r, int vn,
                                   int en) {
        if (vn < 2)
            vn = 2;
        ...
        for (int i = 1; i <= vn; i++) {
            vA[i] = G.addVertex(v.getLabel() + i);
            int j = r.nextInt(i);
            vA[i].addEdge(vA[j], "E" + vA[i].getLabel() + "-" + vA[j].getLabel());
        }
        int e = vn-1;
        int k = 0;

        while (e < en && k < en*en + 100) {

            k++;
            int v1 = r.nextInt(vn+1);
            int v2 = r.nextInt(vn+1);
        ...
        // if (e < en) System.out.println("Varoitus: ei saatu generoitua riittÃ¤vÃ¤sti kaaria: vn=" + vn + " en=" + en + " e=" + e);
     ...
     */
    public static int edgeCount(AbstractGraph G) {
        int e = 0;
    ...
    public static DiGraph shuffleCopy(DiGraph G) {
        getR();
        int n = G.size();
        Vertex[] vA = getVertexArrayIndex(G);
        int[] perm = randomPermutation(n, R.nextInt());
    ...
    /**
     * Rename all vertices to random strings
     * @param G input graph
     */
    public static void randomVertexNames(AbstractGraph G) {
        int n = G.size();
        int nlen = (int)(Math.log10(n)) + 1;
        HashSet<String> S = new HashSet<>(n*2);
        for (Vertex v : G.vertices()) {
            String name;
            do {
                name = randomString(getR(), nlen);
            } while (! S.add(name));
            v.setLabel(name);
        }
    }
